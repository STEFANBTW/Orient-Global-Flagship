...
const LiquidBackground: React.FC = () => {
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none z-0">
      <div className="absolute top-[-10%] left-[-10%] w-[70%] h-[70%] rounded-full bg-primary/10 blur-[140px] animate-liquid" />
      <div className="absolute bottom-[-10%] right-[-10%] w-[60%] h-[60%] rounded-full bg-orange-600/5 blur-[120px] animate-liquid" style={{ animationDelay: '-7s' }} />
      <div className="absolute top-[30%] right-[15%] w-[40%] h-[40%] rounded-full bg-primary/5 blur-[100px] animate-liquid" style={{ animationDelay: '-12s' }} />
      <div className="absolute inset-0 bg-noir-black/20 backdrop-blur-[2px]" />
    </div>
  );
};
...
const Navbar: React.FC<{ 
  ...
  scrolled: boolean, 
  navHidden: boolean 
}> = ({ theme, toggleTheme, setCurrentView, scrolled, navHidden }) => {
    ...
    return (
      <div 
        className={`fixed z-[100] transition-all duration-700 ease-[0.22, 1, 0.36, 1] 
          ${navHidden ? (isMobile ? 'translate-y-32' : '-translate-y-32') : 'translate-y-0'} 
          ${isMobile ? 'bottom-6 left-1/2 -translate-x-1/2 w-[95%]' : 'top-0 left-0 w-full'}`}
      >
        {/* Main Nav Container */}
        <nav className={`relative transition-all duration-500 flex items-center 
          ${isMobile 
            ? `rounded-[40px] h-14 sm:h-16 px-2 sm:px-4 ${scrolled ? 'shadow-soft bg-white/90 dark:bg-zinc-900/90 backdrop-blur-xl border border-black/5 dark:border-white/10' : 'bg-transparent border-transparent'}` 
            : `h-14 sm:h-16 px-4 sm:px-8 ${scrolled ? 'shadow-soft bg-white dark:bg-background-dark backdrop-blur-xl border-b border-black/5 dark:border-white/10' : 'bg-transparent border-transparent shadow-none'}`
          }`}
        >
          <div className={`${isMobile ? 'w-full flex items-center justify-between' : 'lg:max-w-[67vw] mx-auto w-full flex items-center justify-between'}`}>
            {/* Logo Section */}
            <div className={`flex items-center gap-2 sm:gap-3 cursor-pointer ${isMobile ? 'pl-2 pr-4 border-r border-white/10' : ''}`} onClick={() => { setCurrentView('home'); window.scrollTo(0, 0); }}>
              <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-gradient-to-br from-primary to-orange-600 flex items-center justify-center shadow-lg shadow-primary/20">
                <span className="material-icons text-white text-xl sm:text-2xl">diamond</span>
              </div>
              <div className="flex flex-col">
                <span className="font-heading font-black text-sm sm:text-base tracking-tighter dark:text-white text-slate-900 leading-none uppercase">Orient</span>
                <span className="text-[9px] sm:text-[10px] font-bold tracking-[0.3em] text-primary uppercase leading-none mt-0.5">Global</span>
              </div>
            </div>
              ...
              >
                {navLinks.map((link) => (
                  <button 
                    ...
                    className={`relative z-10 px-2 sm:px-3 h-full flex items-center justify-center transition-all duration-300 text-[10px] font-bold uppercase tracking-widest hover:text-primary dark:hover:text-white ${scrolled ? 'dark:text-gray-300 text-slate-600' : 'dark:text-white text-slate-900'}`}
                  >
        ...
        <AnimatePresence>
          {(!isMobile && activeTab) && (
            <motion.div 
              ...
              onMouseLeave={() => handleMouseLeave()}
              className={`absolute w-full border border-black/5 dark:border-white/10 backdrop-blur-xl shadow-[0_40px_80px_rgba(0,0,0,0.5)] z-[-1] overflow-hidden top-full left-0 rounded-none border-t-0 p-12 ${scrolled ? 'bg-white/90 dark:bg-background-dark/80' : 'bg-transparent dark:bg-transparent'}`}
            >
...
const Hero: React.FC = () => {
  ...
  return (
    <section id="hero" className="relative h-[120vh] flex flex-col items-center justify-center text-center px-6 overflow-hidden dark:bg-background-dark bg-slate-100">
      ...
      <motion.div style={{ y: springYText, opacity }} className="relative z-20 max-w-5xl">
        <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, ease: [0.22, 1, 0.36, 1], delay: 2.5 }} className="mb-6">
          <span className="text-primary font-black tracking-[0.4em] uppercase text-xs">Welcome to Orient Global</span>
        </motion.div>
        <div className="font-heading font-black text-6xl md:text-[8.5rem] dark:text-white text-slate-900 mb-8 tracking-tighter leading-[0.8] uppercase">
          <TextReveal text="ELEVATE" delay={2.8} />
          <br />
          <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary via-orange-400 to-gray-500">
             <TextReveal text="EVERYDAY." delay={3.2} />
          </span>
        </div>
        <motion.p 
          ...
          className="text-2xl md:text-3xl dark:text-xl dark:md:text-2xl dark:text-white/90 text-slate-800 max-w-4xl mx-auto font-light tracking-wide mb-16 leading-relaxed"
        >
...
const FinalCTA: React.FC = () => {
  return (
    <section className="relative py-40 px-6 overflow-hidden dark:bg-noir-black bg-white border-t border-black/5 dark:border-white/5">
      {/* Pulsing Ambient Glows */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-primary/20 rounded-full blur-[120px] animate-pulse" />
        <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-orange-600/10 rounded-full blur-[120px] animate-pulse" style={{ animationDelay: '2s' }} />
      </div>

      <div className="relative z-10 lg:max-w-[67vw] mx-auto text-center">
        <Reveal>
          <div className="inline-block px-6 py-2 rounded-full bg-primary/10 border border-primary/20 mb-8">
            <span className="text-primary font-black uppercase tracking-[0.4em] text-[10px]">The New Standard of Excellence</span>
          </div>
        </Reveal>

        <h2 className="text-7xl md:text-9xl font-black dark:text-white text-slate-900 mb-8 leading-[0.9] uppercase tracking-tighter">
          <TextReveal text="ORIENT" /><br />
          <span className="text-primary italic font-serif lowercase tracking-normal">Global</span>
        </h2>

        <Reveal>
          <p className="text-xl dark:text-gray-400 text-slate-500 max-w-2xl mx-auto leading-relaxed font-medium mb-12">
        ...
        <Reveal>
          <div className="flex flex-wrap justify-center gap-6">
            <button className="px-10 py-4 bg-primary text-white font-black uppercase tracking-widest rounded-2xl shadow-2xl shadow-primary/30 hover:scale-105 transition-all">Visit Us Today</button>
          </div>
        </Reveal>
      </div>
    </section>
...
const BakeryShowcase: React.FC<{ setCurrentView: (v: any) => void }> = ({ setCurrentView }) => {
  return (
    <section id="bakery" className="py-40 px-4 md:px-12 lg:max-w-[67vw] mx-auto dark:bg-background-dark bg-white transition-colors duration-500">
      <div className="flex flex-col md:flex-row gap-16 items-start">
        <div className="md:w-1/3 pt-12">
          <span className="text-primary font-bold tracking-[0.4em] uppercase text-xs mb-4 block">The Bakery</span>
          <h2 className="text-6xl font-black dark:text-white text-slate-900 mb-8 font-heading leading-tight uppercase tracking-tighter">
            <TextReveal text="GOLDEN" /><br/><TextReveal text="CRUST" delay={0.1} />
          </h2>
          <p className="dark:text-gray-400 text-slate-500 text-lg leading-relaxed mb-10 font-medium">Morning excellence starts here. From artisanal sourdoughs to delicate pastries, our bakery combines European techniques with local flavors like Coconut and Plantain.</p>
          <button onClick={() => { setCurrentView('bakery'); window.scrollTo(0, 0); }} className="group flex items-center gap-3 dark:text-white text-slate-900 border-b-2 border-primary pb-2 hover:text-primary transition-all uppercase text-sm font-black tracking-[0.2em] font-heading"><span>View Menu</span><span className="material-icons text-lg group-hover:translate-x-1 transition-transform">arrow_forward</span></button>
        </div>
        <div className="md:w-2/3 grid grid-cols-1 gap-12"><ParallaxImage src="https://images.unsplash.com/photo-1555507036-ab1f4038808a?q=80&w=1926&auto=format&fit=crop" alt="Golden Crust Croissants" className="rounded-[3rem] h-[75vh] border border-white/5 shadow-2xl" /><ParallaxImage src="https://images.unsplash.com/photo-1517686469429-8bdb88b9f907?q=80&w=2070&auto=format&fit=crop" alt="Bakery Details" className="rounded-[3rem] h-[45vh] border border-white/5 shadow-2xl" /></div>
      </div>
    </section>
  );
};
...
const BakeryDeepDive: React.FC = () => {
  return (
    <section className="py-40 px-6 bg-slate-100 dark:bg-noir-black border-t border-black/5 dark:border-white/5">
      <div className="lg:max-w-[67vw] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-24 items-center">
        <Reveal animation='slide-from-left' className="order-2 lg:order-1">
          <ParallaxImage src="https://images.unsplash.com/photo-1509440159596-0249088772ff?q=80&w=2072&auto=format&fit=crop" alt="Baking Process" className="rounded-[3rem] aspect-square border border-white/10 shadow-3xl" />
        </Reveal>
        <Reveal animation='slide-from-right' className="order-1 lg:order-2">
          ...
          <h2 className="text-4xl sm:text-6xl font-black dark:text-white text-slate-900 uppercase tracking-tighter mb-8">THE SCIENCE OF <span className="dark:text-white/40 text-slate-400 italic">FERMENTATION.</span></h2>
          <p className="text-xl dark:text-gray-400 text-slate-600 leading-relaxed mb-8">Our master bakers utilize a 48-hour cold fermentation process, allowing complex flavors to develop naturally. We source our grains from sustainable farms, ensuring every loaf meets the Orient Global standard of purity.</p>
          <ul className="space-y-4">
            {['Natural Sourdough Starters', 'Stone-Ground Flour', 'No Artificial Additives'].map((item, i) => (
              <Reveal key={item} delay={`delay-${i * 150}`}>
                <li className="flex items-center gap-4 dark:text-white text-slate-900 font-bold uppercase tracking-widest text-sm">
                  <span className="w-2 h-2 rounded-full bg-primary shadow-[0_0_10px_rgba(242,158,13,0.5)]" />
                  {item}
                </li>
              </Reveal>
            ))}
...
const MarketShowcase: React.FC<{ setCurrentView: (v: any) => void }> = ({ setCurrentView }) => {
  return (
    <section id="market" className="py-32 px-4 md:px-12 lg:max-w-[67vw] mx-auto dark:bg-background-dark bg-white transition-colors duration-500">
      <div className="flex flex-col md:flex-row-reverse gap-16 items-start">
        <div className="md:w-1/3 pt-12 text-right md:text-left">
          <span className="text-primary font-bold tracking-[0.4em] uppercase text-xs mb-4 block">The Market</span>
          <h2 className="text-6xl font-black dark:text-white text-slate-900 mb-8 font-heading uppercase leading-tight tracking-tighter">
             <TextReveal text="GLOBAL" /><br/><TextReveal text="STANDARDS" delay={0.1} />
          </h2>
          <p className="dark:text-gray-400 text-slate-500 text-lg leading-relaxed mb-10 font-medium">We've redefined the shopping experience. Wide aisles, perfect lighting, and a curated selection of international and local products. Quality you can trust, right here in Rayfield, Jos.</p>
          <div className="flex justify-end md:justify-start"><button onClick={() => { setCurrentView('supermarket'); window.scrollTo(0, 0); }} className="group flex items-center gap-3 dark:text-white text-slate-900 border-b-2 border-primary pb-2 hover:text-primary transition-all uppercase text-sm font-black tracking-[0.2em] font-heading"><span>Shop Online</span><span className="material-icons text-lg group-hover:translate-x-1 transition-transform">arrow_forward</span></button></div>
        </div>
        <div className="md:w-2/3 grid grid-cols-2 gap-8"><ParallaxImage src="https://images.unsplash.com/photo-1542838132-92c53300491e?q=80&w=1974&auto=format&fit=crop" alt="Supermarket Wide" className="rounded-[3rem] h-[50vh] col-span-2 border border-white/5 shadow-2xl" /><ParallaxImage src="https://images.unsplash.com/photo-1543083477-4f7f4b23832c?q=80&w=1970&auto=format&fit=crop" alt="Fresh Produce" className="rounded-[3rem] h-[40vh] border border-white/5 shadow-2xl" /><ParallaxImage src="https://images.unsplash.com/photo-1506484334402-40ff22e05a6d?q=80&w=2070&auto=format&fit=crop" alt="Market Shelves" className="rounded-[3rem] h-[40vh] border border-white/5 shadow-2xl" /></div>
      </div>
    </section>
  );
};
    ...
    <section className="py-40 px-6 bg-slate-100 dark:bg-noir-black border-t border-black/5 dark:border-white/5">
      <div className="lg:max-w-[67vw] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-24 items-center">
        <Reveal animation='slide-from-right'>
          <span className="text-primary font-black tracking-[0.5em] uppercase text-[10px] mb-4 block">Supply Chain</span>
          <h2 className="text-4xl sm:text-6xl font-black dark:text-white text-slate-900 uppercase tracking-tighter mb-8">INSTITUTIONAL <span className="dark:text-white/40 text-slate-400 italic">QUALITY.</span></h2>
          <p className="text-xl dark:text-gray-400 text-slate-600 leading-relaxed mb-8">Our global procurement network ensures that the finest products from around the world are available in Jos. From organic dairy to international spices, we maintain a strict cold chain and quality control protocol.</p>
          <div className="grid grid-cols-2 gap-6">
            <Reveal delay='delay-75'>
              <div className="p-6 rounded-3xl dark:bg-white/5 bg-black/5 border dark:border-white/10 border-black/10">
                ...
                <p className="dark:text-gray-500 text-slate-500 text-xs">24/7 Temperature Monitoring</p>
              </div>
            </Reveal>
            <Reveal delay='delay-150'>
              <div className="p-6 rounded-3xl dark:bg-white/5 bg-black/5 border dark:border-white/10 border-black/10">
                ...
                <p className="dark:text-gray-500 text-slate-500 text-xs">Direct from Global Producers</p>
              </div>
            </Reveal>
          </div>
        </Reveal>
...
const RestaurantShowcase: React.FC<{ setCurrentView: (v: any) => void }> = ({ setCurrentView }) => {
  return (
    <section id="restaurant" className="py-40 px-4 md:px-12 lg:max-w-[67vw] mx-auto dark:bg-background-dark bg-white transition-colors duration-500">
      <div className="flex flex-col md:flex-row gap-16 items-start">
        <div className="md:w-1/3 pt-12">
          <span className="text-primary font-bold tracking-[0.4em] uppercase text-xs mb-4 block">The Restaurant</span>
          <h2 className="text-6xl font-black dark:text-white text-slate-900 mb-8 font-heading leading-tight uppercase tracking-tighter">
            <TextReveal text="CULINARY" /><br/><TextReveal text="ARTISTRY" delay={0.1} />
          </h2>
          <p className="dark:text-gray-400 text-slate-500 text-lg leading-relaxed mb-10 font-medium">A symphony of flavors crafted by world-class chefs. From local delicacies like Pounded Yam & Egusi to international fusion, every dish is a masterpiece of taste and presentation.</p>
          <button onClick={() => { setCurrentView('dining'); window.scrollTo(0, 0); }} className="group flex items-center gap-3 dark:text-white text-slate-900 border-b-2 border-primary pb-2 hover:text-primary transition-all uppercase text-sm font-black tracking-[0.2em] font-heading"><span>View Menu</span><span className="material-icons text-lg group-hover:translate-x-1 transition-transform">arrow_forward</span></button>
        </div>
        <div className="md:w-2/3 grid grid-cols-1 gap-12">
          <ParallaxImage src="https://images.unsplash.com/photo-1514362545857-3bc16c4c7d1b?q=80&w=2070&auto=format&fit=crop" alt="Fine Dining Experience" className="rounded-[3rem] h-[75vh] border border-white/5 shadow-2xl" />
        </div>
      </div>
    </section>
  );
...
const DiningDeepDive: React.FC = () => {
  return (
    <section className="py-40 px-6 bg-slate-100 dark:bg-noir-black border-t border-white/5">
      <div className="lg:max-w-[67vw] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-24 items-center">
        <Reveal animation='rotate-in' className="order-2 lg:order-1 relative aspect-square rounded-[3rem] overflow-hidden border border-white/10 shadow-3xl">
        ...
        <Reveal animation='slide-from-right' className="order-1 lg:order-2">
          <span className="text-primary font-black tracking-[0.5em] uppercase text-[10px] mb-4 block">Chef's Philosophy</span>
          <h2 className="text-4xl sm:text-6xl font-black dark:text-white text-slate-900 uppercase tracking-tighter mb-8">FUSION OF <span className="dark:text-white/40 text-slate-400 italic">HERITAGE.</span></h2>
          <p className="text-xl dark:text-gray-400 text-slate-600 leading-relaxed mb-8">Our culinary team explores the intersection of traditional Plateau ingredients and modern gastronomic techniques. We believe in "Root-to-Table" dining, where every ingredient tells a story of the land.</p>
          <div className="space-y-6">
            {['Locally Sourced Produce', 'Artisanal Plating', 'Curated Wine Pairings'].map(item => (
              <div key={item} className="flex items-center gap-6 p-6 rounded-3xl dark:bg-white/5 bg-black/5 border dark:border-white/10 border-black/10">
                <span className="material-icons text-primary">restaurant_menu</span>
                <span className="dark:text-white text-slate-900 font-bold uppercase tracking-widest text-sm">{item}</span>
              </div>
            ))}
          </div>
        </Reveal>
...
const WaterShowcase: React.FC<{ setCurrentView: (v: any) => void }> = ({ setCurrentView }) => {
  return (
    <section id="water" className="py-32 px-4 md:px-12 lg:max-w-[67vw] mx-auto dark:bg-background-dark bg-white transition-colors duration-500">
      <div className="flex flex-col md:flex-row-reverse gap-16 items-start">
        <div className="md:w-1/3 pt-12 text-right md:text-left">
          <span className="text-primary font-bold tracking-[0.4em] uppercase text-xs mb-4 block">Orient Water</span>
          <h2 className="text-6xl font-black dark:text-white text-slate-900 mb-8 font-heading uppercase leading-tight tracking-tighter">
             <TextReveal text="PURE" /><br/><TextReveal text="HYDRATION" delay={0.1} />
          </h2>
          <p className="dark:text-gray-400 text-slate-500 text-lg leading-relaxed mb-10 font-medium">The gold standard of purity. Our 7-step filtration process ensures every drop is as crisp and clean as nature intended. NAFDAC Certified quality for your peace of mind.</p>
          <div className="flex justify-end md:justify-start"><button onClick={() => { setCurrentView('water'); window.scrollTo(0, 0); }} className="group flex items-center gap-3 dark:text-white text-slate-900 border-b-2 border-primary pb-2 hover:text-primary transition-all uppercase text-sm font-black tracking-[0.2em] font-heading"><span>Explore Purity</span><span className="material-icons text-lg group-hover:translate-x-1 transition-transform">arrow_forward</span></button></div>
        </div>
        <div className="md:w-2/3 grid grid-cols-2 gap-8">
          <ParallaxImage src="https://images.unsplash.com/photo-1548839140-29a749e1cf4d?q=80&w=1888&auto=format&fit=crop" alt="Orient Water Bottle" className="rounded-[3rem] h-[60vh] col-span-2 border border-white/5 shadow-2xl" />
        </div>
      </div>
    </section>
  );
...
const WaterDeepDive: React.FC = () => {
  return (
    <section className="py-40 px-6 bg-slate-100 dark:bg-noir-black border-t border-white/5">
      <div className="lg:max-w-[67vw] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-24 items-center">
        <Reveal animation='slide-from-left'>
          <span className="text-primary font-black tracking-[0.5em] uppercase text-[10px] mb-4 block">Technical Purity</span>
          <h2 className="text-4xl sm:text-6xl font-black dark:text-white text-slate-900 uppercase tracking-tighter mb-8">7-STEP <span className="dark:text-white/40 text-slate-400 italic">FILTRATION.</span></h2>
          <p className="text-xl dark:text-gray-400 text-slate-600 leading-relaxed mb-8">Beyond standard purification, Orient Water undergoes a rigorous 7-step process including Reverse Osmosis, UV Sterilization, and Ozone Treatment. We test every batch in our on-site laboratory to ensure absolute safety.</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {['Reverse Osmosis', 'UV Sterilization', 'Ozone Treatment', 'Mineral Balancing'].map(step => (
              <div key={step} className="flex items-center gap-4 p-4 rounded-2xl dark:bg-white/5 bg-black/5 border dark:border-white/10 border-black/10">
                <span className="w-2 h-2 rounded-full bg-sky-500" />
                <span className="dark:text-white text-slate-900 text-xs font-black uppercase tracking-widest">{step}</span>
              </div>
            ))}
          </div>
        </Reveal>
...
const CuratedExperiences: React.FC<{ setCurrentView: (v: any) => void }> = ({ setCurrentView }) => {
  return (
    <section id="lounge" className="relative py-32 dark:bg-background-dark bg-white transition-colors duration-500">
      <div className="container mx-auto px-6">
        <div className="mb-24 text-center">
          <h2 className="text-primary text-xs font-black uppercase tracking-[0.6em] mb-4">Our Worlds</h2>
          <h3 className="text-6xl md:text-8xl font-serif dark:text-white text-slate-900 italic font-light tracking-tight">Curated <span className="not-italic font-display font-black dark:text-gray-500 text-slate-400">Experiences</span></h3>
        </div>
        <div className="flex flex-col md:flex-row items-center gap-20">
          <div className="w-full md:w-1/2"><ParallaxImage src="https://images.unsplash.com/photo-1470337458703-46ad1756a187?q=80&w=2069&auto=format&fit=crop" alt="Lounge Experience" className="rounded-[4rem] aspect-[4/5] border border-white/5 shadow-[0_50px_100px_rgba(0,0,0,0.5)]" /></div>
          <div className="w-full md:w-1/2">
            <div className="w-16 h-1.5 bg-primary mb-12 rounded-full"></div>
            <h4 className="text-5xl font-black dark:text-white text-slate-900 mb-8 uppercase tracking-tighter leading-none"><TextReveal text="The Nightscape Lounge" /></h4>
            <p className="dark:text-gray-400 text-slate-500 text-xl leading-relaxed mb-10 font-medium">Where mixology meets mystery. Our lounge offers a secluded environment perfect for high-stakes meetings or unwinding after a long week. Featuring Zobo-infused cocktails and a curated cigar selection.</p>
            <ul className="space-y-6 mb-12">{['Premium Bottle Service', 'Private Booths', 'Live Jazz Weekends'].map(li => (<li key={li} className="flex items-center dark:text-gray-300 text-slate-600 font-bold text-lg tracking-wide uppercase"><span className="w-3 h-3 bg-primary rounded-full mr-6 shadow-[0_0_15px_rgba(242,158,13,0.5)]"></span>{li}</li>))}</ul>
            <button onClick={() => { setCurrentView('lounge'); window.scrollTo(0, 0); }} className="inline-flex items-center text-primary font-black tracking-[0.3em] uppercase text-sm hover:text-black dark:hover:text-white transition-all border-b-2 border-primary pb-1 font-heading">Explore Menu <span className="material-icons ml-3 text-lg">arrow_forward</span></button>
          </div>
        </div>
      </div>
    </section>
...
const LoungeDeepDive: React.FC = () => {
  return (
    <section className="py-40 px-6 bg-white dark:bg-noir-black border-t border-black/5 dark:border-white/5">
      <div className="lg:max-w-[67vw] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-24 items-center">
        <Reveal animation='slide-from-left' className="order-2 lg:order-1 relative aspect-square rounded-[3rem] overflow-hidden border border-white/10 shadow-3xl">
        ...
        <Reveal animation='slide-from-right' className="order-1 lg:order-2">
          <span className="text-primary font-black tracking-[0.5em] uppercase text-[10px] mb-4 block">Mixology Art</span>
          <h2 className="text-4xl sm:text-6xl font-black dark:text-white text-slate-900 uppercase tracking-tighter mb-8">THE NIGHTSCAPE <span className="dark:text-white/40 text-slate-400 italic">ETHOS.</span></h2>
          <p className="text-xl dark:text-gray-400 text-slate-600 leading-relaxed mb-8">Our mixologists are alchemists, blending local botanicals with premium spirits to create unique sensory experiences. The Nightscape Lounge is more than a bar; it's a sanctuary for the discerning.</p>
          <div className="grid grid-cols-2 gap-4">
            {['Signature Cocktails', 'Rare Spirits', 'Bespoke Service', 'Acoustic Excellence'].map((item, i) => (
              <Reveal key={item} delay={`delay-${i * 100}`}>
                <div className="p-6 rounded-3xl dark:bg-white/5 bg-black/5 border dark:border-white/10 border-black/10 text-center">
...
const GamesShowcase: React.FC<{ setCurrentView: (v: any) => void }> = ({ setCurrentView }) => {
  return (
    <section id="games" className="py-32 px-4 md:px-12 lg:max-w-[67vw] mx-auto dark:bg-background-dark bg-white transition-colors duration-500">
      <div className="flex flex-col md:flex-row gap-16 items-start">
        <div className="md:w-1/3 pt-12">
          <span className="text-primary font-bold tracking-[0.4em] uppercase text-xs mb-4 block">The Arena</span>
          <h2 className="text-6xl font-black dark:text-white text-slate-900 mb-8 font-heading leading-tight uppercase tracking-tighter">
            <TextReveal text="HIGH" /><br/><TextReveal text="PERFORMANCE" delay={0.1} />
          </h2>
          <p className="dark:text-gray-400 text-slate-500 text-lg leading-relaxed mb-10 font-medium">Step into the future of competitive gaming. From high-refresh rate PC arenas to immersive VR pods, we provide the infrastructure for champions.</p>
          <button onClick={() => { setCurrentView('games'); window.scrollTo(0, 0); }} className="group flex items-center gap-3 dark:text-white text-slate-900 border-b-2 border-primary pb-2 hover:text-primary transition-all uppercase text-sm font-black tracking-[0.2em] font-heading"><span>Enter Arena</span><span className="material-icons text-lg group-hover:translate-x-1 transition-transform">arrow_forward</span></button>
        </div>
        <div className="md:w-2/3 grid grid-cols-1 gap-12">
          <ParallaxImage src="https://images.unsplash.com/photo-1542751371-adc38448a05e?q=80&w=2070&auto=format&fit=crop" alt="Gaming Arena" className="rounded-[3rem] h-[60vh] border border-white/5 shadow-2xl" />
        </div>
      </div>
    </section>
  );
...
const GamesDeepDive: React.FC = () => {
  return (
    <section className="py-40 px-6 bg-slate-100 dark:bg-noir-black border-t border-white/5">
      <div className="lg:max-w-[67vw] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-24 items-center">
        <Reveal animation='slide-from-right'>
          <span className="text-primary font-black tracking-[0.5em] uppercase text-[10px] mb-4 block">Tech Ecosystem</span>
          <h2 className="text-4xl sm:text-6xl font-black dark:text-white text-slate-900 uppercase tracking-tighter mb-8">ZERO LAG <span className="dark:text-white/40 text-slate-400 italic">INFRASTRUCTURE.</span></h2>
          <p className="text-xl dark:text-gray-400 text-slate-600 leading-relaxed mb-8">We've built a dedicated fiber-optic network to ensure sub-10ms latency for competitive play. Our hardware is refreshed quarterly, featuring the latest RTX GPUs and high-fidelity VR peripherals.</p>
          <div className="space-y-4">
            {['Fiber-Optic Backbone', 'RTX 40-Series GPUs', '240Hz Displays'].map(spec => (
              <div key={spec} className="flex items-center justify-between p-6 rounded-3xl dark:bg-white/5 bg-black/5 border dark:border-white/10 border-black/10">
...
const VoicesOfJos: React.FC = () => {
  ...
  return (
    <section id="voices" className="py-40 bg-slate-50 dark:bg-noir-black border-y border-black/5 dark:border-white/5 transition-colors duration-500">
      <div className="lg:max-w-[67vw] mx-auto px-4 md:px-12">
        <Reveal>
          <div className="text-center mb-24">
            <h2 className="text-5xl font-black dark:text-white text-slate-900 mb-6 uppercase tracking-tighter"><TextReveal text="VOICES OF JOS" /></h2>
            <p className="dark:text-gray-400 text-slate-500 font-bold text-lg uppercase tracking-widest">Real stories from our community.</p>
          </div>
        </Reveal>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
          {reviews.map((rev, idx) => (
            <Reveal key={rev.name} delay={`delay-${idx * 150}`}>
              <motion.div whileHover={{ y: -10 }} className="dark:bg-background-dark bg-white p-10 rounded-[3rem] border border-black/5 dark:border-white/5 hover:border-primary/50 transition-all group shadow-soft hover:shadow-elite">
                <div className="flex items-center gap-6 mb-10"><img src={rev.img} className="w-16 h-16 rounded-full border-2 border-primary/20 object-cover" alt={rev.name} /><div><h4 className="dark:text-white text-slate-900 font-black text-lg uppercase tracking-tight">{rev.name}</h4><p className="text-[10px] dark:text-gray-500 text-slate-400 uppercase font-black tracking-[0.3em]">{rev.role}</p></div></div>
                <div className="dark:bg-zinc-800/50 bg-slate-50 rounded-[2rem] p-5 flex items-center gap-6 mb-8"><button className="w-12 h-12 rounded-full bg-primary flex items-center justify-center text-white shadow-xl hover:bg-orange-600 transition-colors"><span className="material-icons text-2xl">play_arrow</span></button><div className="flex gap-1.5 h-10 items-center flex-1">{[...Array(12)].map((_, i) => (<div key={i} className="w-1 bg-primary/40 rounded-full wave-bar" style={{ animationDelay: `${i * 0.1}s` }}></div>))}</div><span className="text-xs text-gray-500 font-mono font-bold">0:24</span></div>
                <p className="dark:text-gray-300 text-slate-500 text-lg italic font-medium leading-relaxed">"{rev.text}"</p>
              </motion.div>
            </Reveal>
          ))}
        </div>
...
const ServicesGrid: React.FC<{ setCurrentView: (v: any) => void }> = ({ setCurrentView }) => {
  ...
  return (
    <section id="services" className="relative z-30 dark:bg-noir-black bg-white py-40 px-4 sm:px-8">
      <div className="lg:max-w-[67vw] mx-auto">
        <Reveal>
          <span className="text-primary font-black tracking-[0.5em] uppercase text-[10px] mb-4 block">
            Bespoke Services
          </span>
          <h2 className="text-4xl sm:text-6xl font-black dark:text-white text-slate-900 uppercase tracking-tighter max-w-3xl">
        ...
        <motion.div 
          layout 
          className="grid gap-6 w-full h-full min-h-[70vh]" 
          style={getGridStyles()}
          transition={gridTransition}
        >
          <Reveal delay='delay-75' className='h-full'>
            <MagneticCard 
              id="card-bakery" 
              title="Bakery" 
...
const TrustSection: React.FC = () => {
  ...
  return (
    <section className="py-40 px-6 relative overflow-hidden dark:bg-noir-black bg-white border-t border-black/5 dark:border-white/5">
      <div className="lg:max-w-[67vw] mx-auto">
        <Reveal>
          <div className="text-center mb-24">
            <span className="text-primary font-black tracking-[0.5em] uppercase text-[10px] mb-4 block">
            ...
            <h2 className="text-4xl sm:text-6xl font-black dark:text-white text-slate-900 uppercase tracking-tighter">
              UNCOMPROMISING <span className="dark:text-white/40 text-slate-300 italic">QUALITY.</span>
            </h2>
          </div>
        </Reveal>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
          {certifications.map((cert, i) => (
            <Reveal key={cert.name} delay={`delay-${i * 150}`}>
              <div 
                className="p-12 rounded-[3rem] bg-white dark:bg-zinc-900/40 border border-black/5 dark:border-white/5 shadow-soft group hover:border-primary/30 transition-all duration-700"
              >
                <div className="w-16 h-16 rounded-2xl bg-primary/10 flex items-center justify-center mb-8 group-hover:scale-110 transition-transform duration-500">
                  <span className="material-icons text-primary text-4xl">{cert.icon}</span>
                </div>
                <h3 className="text-2xl font-black dark:text-white text-slate-900 uppercase mb-4 tracking-tight">{cert.name}</h3>
                <p className="dark:text-gray-400 text-slate-600 leading-relaxed font-medium">{cert.desc}</p>
              </div>
            </Reveal>
          ))}
        </div>
...
const LocationSection: React.FC = () => {
  return (
    <section id="location" className="py-40 px-6 bg-slate-50 dark:bg-noir-black border-t border-black/5 dark:border-white/5">
      <div className="lg:max-w-[67vw] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-24 items-center">
        <Reveal animation='slide-from-left'>
          <span className="text-primary font-black tracking-[0.5em] uppercase text-[10px] mb-4 block">
            Our Presence
          </span>
          <h2 className="text-4xl sm:text-6xl font-black dark:text-white text-slate-900 uppercase tracking-tighter mb-8">
            RAYFIELD, <span className="dark:text-white/40 text-slate-300 italic">JOS.</span>
          </h2>
          <p className="text-xl dark:text-gray-400 text-slate-600 leading-relaxed mb-12">
            Visit our flagship destination at Amanda Plaza. A convergence of all lifestyle divisions in the heart of Plateau State.
          </p>
          <div className="space-y-6">
            <Reveal delay='delay-75'>
              <div className="flex items-center gap-6 p-8 rounded-3xl dark:bg-white/5 bg-slate-50 border border-black/5 dark:border-white/10 shadow-soft">
                <span className="material-icons text-primary text-3xl">location_on</span>
                <div>
                  <h4 className="dark:text-white text-slate-900 font-black uppercase tracking-tight">Amanda Plaza</h4>
                  <p className="dark:text-gray-500 text-slate-400 text-sm">Rayfield, Jos, Plateau State</p>
                </div>
              </div>
            </Reveal>
            <Reveal delay='delay-150'>
              <div className="flex items-center gap-6 p-8 rounded-3xl dark:bg-white/5 bg-slate-50 border border-black/5 dark:border-white/10 shadow-soft">
                ...
                <div>
                  <h4 className="dark:text-white text-slate-900 font-black uppercase tracking-tight">Open Daily</h4>
                  <p className="dark:text-gray-500 text-slate-400 text-sm">08:00 AM — 10:00 PM</p>
                </div>
              </div>
            </Reveal>
          </div>
        ...
        <Reveal animation='slide-from-right'>
          <div className="relative aspect-square rounded-[3rem] overflow-hidden border border-white/10 shadow-2xl">
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="p-8 rounded-full bg-primary/20 backdrop-blur-xl border border-primary/30 animate-pulse">
                <span className="material-icons text-primary text-5xl">explore</span>
              </div>
            </div>
          </div>
        </Reveal>
...
const Footer: React.FC<{ setCurrentView: (v: any) => void }> = ({ setCurrentView }) => {
  ...
  return (
    <footer className="bg-slate-100 dark:bg-noir-black dark:text-white text-slate-900 py-24 border-t border-black/5 dark:border-white/10">
      <div className="container mx-auto px-4 md:px-12">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-16 mb-24">
          <div className="col-span-1 lg:col-span-2">
            <div className="flex items-center gap-2 mb-8">
              <div className="w-10 h-10 rounded bg-gradient-to-br from-primary to-orange-600 flex items-center justify-center text-white font-heading font-black text-xl shadow-lg shadow-primary/20">O</div>
              <span className="font-heading font-black text-2xl tracking-tight dark:text-white text-slate-900 uppercase">Orient Global Services</span>
            </div>
            <p className="dark:text-gray-400 text-slate-500 text-sm leading-relaxed mb-10 font-medium max-w-md">
            ...
            <div className="flex gap-4">
              <a className="w-10 h-10 rounded-full dark:bg-white/5 bg-slate-100 dark:text-white text-slate-900 hover:bg-primary hover:text-black flex items-center justify-center transition-all shadow-soft" href="#"><i className="material-icons text-lg">facebook</i></a>
              <a className="w-10 h-10 rounded-full dark:bg-white/5 bg-slate-100 dark:text-white text-slate-900 hover:bg-primary hover:text-black flex items-center justify-center transition-all shadow-soft" href="#"><i className="material-icons text-lg">camera_alt</i></a>
              <a className="w-10 h-10 rounded-full dark:bg-white/5 bg-slate-100 dark:text-white text-slate-900 hover:bg-primary hover:text-black flex items-center justify-center transition-all shadow-soft" href="#"><i className="material-icons text-lg">alternate_email</i></a>
              <a className="w-10 h-10 rounded-full dark:bg-white/5 bg-slate-100 dark:text-white text-slate-900 hover:bg-primary hover:text-black flex items-center justify-center transition-all shadow-soft" href="#"><i className="material-icons text-lg">business</i></a>
            </div>
          </div>
          
          <div>
            <h3 className="dark:text-white text-slate-900 font-black uppercase tracking-[0.2em] mb-8 text-xs">Divisions</h3>
            <ul className="space-y-4 dark:text-gray-400 text-slate-500 text-xs font-bold uppercase tracking-widest">
              {[
          ...
          <div>
            <h3 className="text-white font-black uppercase tracking-[0.2em] mb-8 text-xs">Corporate</h3>
          ...
          <div>
            <h3 className="text-white font-black uppercase tracking-[0.2em] mb-8 text-xs">Newsletter</h3>
            <p className="text-xs text-gray-400 mb-6 leading-relaxed">Subscribe to receive corporate updates and exclusive offers.</p>
            <div className="relative">
              <input className="w-full bg-white/5 border border-white/10 rounded-xl py-3 px-4 text-white placeholder-gray-500 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all text-sm" placeholder="Email Address" type="email"/>
            ...
            <p className="text-[10px] text-gray-600 mt-4 uppercase tracking-widest">By subscribing, you agree to our Privacy Policy.</p>
          </div>
        </div>
        
        <div className="border-t border-white/10 pt-8 flex flex-col md:flex-row justify-between items-center gap-6 text-[10px] text-gray-500 uppercase tracking-[0.2em] font-bold">
...
const ChatBot: React.FC = () => {
  ...
  return (
    <div className="fixed bottom-6 right-6 sm:bottom-12 sm:right-12 z-[100]">
      <AnimatePresence>
        {isOpen && (
          <motion.div initial={{ opacity: 0, scale: 0.9, y: 50 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 50 }} className="absolute bottom-20 sm:bottom-24 right-0 w-[calc(100vw-3rem)] sm:w-[400px] h-[60vh] sm:h-[600px] max-h-[600px] rounded-[2rem] sm:rounded-[3rem] shadow-[0_50px_100px_rgba(0,0,0,0.8)] flex flex-col overflow-hidden dark:bg-zinc-900 bg-white">
...
const App: React.FC = () => {
  ...
  return (
    <>
        ...
        <AmbientGlow color={getAmbientColor()} />
        ...
        {currentView === 'water' && (
          <>
            ...
            <WaterApp currentPage={waterPage} />
            <Footer setCurrentView={setCurrentView} />
          </>
        )}
        {currentView === 'lounge' && (
          <>
            ...
            <LoungeApp currentPage={loungePage} />
            <Footer setCurrentView={setCurrentView} />
          </>
        )}
        {currentView === 'about' && (
