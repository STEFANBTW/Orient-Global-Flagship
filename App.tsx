...
const Preloader: React.FC<{ onComplete: () => void }> = ({ onComplete }) => {
 ...
 className={`fixed inset-0 z-[100] ${theme === 'dark' ? 'bg-black' : 'bg-card'} flex items-center justify-center`}
 >
 ...
 >
 <div className="absolute inset-0 rounded-2xl backdrop-blur-sm bg-card/10"></div>
 <div className="w-10 h-10 border-[6px] border-transparent rounded-full relative z-10"></div>
 </motion.div>
...
const Navbar: React.FC<{ 
 ...
 const getNavBackgroundStyle = () => {
  ...
  if (isHomepage) {
    if (activeTab) {
      // 100% transparent + dense blur on hover
      return 'bg-transparent backdrop-blur-[20px] border-b border-white/10';
    }
 ...
 menu: {
 navigation: [
 { name: 'Traditional', id: 'dining-deep' },
 { name: 'Continental', id: 'dining-deep' },
 { name: 'Wine List', id: 'dining-deep' }
 ],
 highlights: [
 { title: 'Chef Table', detail: 'Private culinary experiences' },
 { title: 'Farm to Fork', detail: 'Locally sourced ingredients' },
 { title: 'Sommelier Choice', detail: 'Expertly curated pairings' }
 ]
 }
 },
 ...
 return (
 <motion.div 
 initial={isMobile ? { y: 80, scale: 0.95, opacity: 0 } : { y: -80, opacity: 0 }}
 animate={{ 
 y: (navHidden && !isHomepage) ? (isMobile ? 128 : -128) : 0,
 scale: (navHidden && !isHomepage) && isMobile ? 0.95 : 1,
 opacity: (navHidden && !isHomepage) ? 0 : 1
 }}
 transition={{ 
 duration: 0.6, ease: [0.16, 1, 0.3, 1]
 }}
 className={`fixed z-[1000] ${isMobile ? 'bottom-3 left-0 right-0 mx-auto w-[90%]' : 'top-0 left-0 w-full'}`}
 >
 {/* Main Nav Container */}
 ...
 <button 
 onClick={() => setCurrentView('login')} className={`p-1.5 sm:p-2 rounded-full transition-all duration-300 text-foreground hover:text-primary`}
 title="User Portal"
 >
 <span className="material-icons text-sm sm:text-base">person</span>
 </button>
 <button 
 onClick={() => setCurrentView('admin')} className={`p-1.5 sm:p-2 rounded-full transition-all duration-300 text-foreground hover:text-primary`}
 title="Admin Portal"
 >
 ...
 onMouseLeave={() => handleMouseLeave()}
 className={`absolute w-full border border-transparent shadow-[0_32px_64px_rgba(0,0,0,0.4)] z-[-1] overflow-hidden top-full left-0 rounded-none border-t-0 p-12 ${getMegaMenuBackgroundStyle()}`}
 >
 ...
 className={`text-xl sm:text-2xl font-black transition-all hover:translate-x-3 flex items-center gap-4 group uppercase tracking-tight text-foreground hover:text-primary`}
 >
 ...
 <div className="space-y-8">
 {activeLink.menu.highlights.map((highlight, i) => (
 <motion.div 
 key={i}
 ...
 >
 <h5 
 className={`text-sm font-black uppercase tracking-widest mb-1 transition-colors text-foreground group-hover:text-primary`}
 >
 {highlight.title}
 </h5>
 <p 
 ...
 <h4 className="text-primary font-black text-[10px] uppercase tracking-[0.2em] border-b border-primary/10 pb-2">Navigation</h4>
 ...
 className="block w-full text-left text-sm font-bold text-foreground/80 uppercase tracking-widest"
 >
 {item.name}
 </button>
 ))}
 ...
 <div className="space-y-4">
 <h4 className="text-primary font-black text-[10px] uppercase tracking-[0.2em] border-b border-primary/10 pb-2">Insights</h4>
 {link.menu.highlights.map((highlight, i) => (
 <div key={i} className="space-y-1">
 <h5 className="text-[10px] font-black text-foreground uppercase tracking-widest">{highlight.title}</h5>
...
const Hero: React.FC<{ isReady: boolean; isActive?: boolean; skipAnimation?: boolean; cms?: any }> = ({ isReady, isActive = false, skipAnimation = false, cms }) => {
  const headline = cms?.headline || "ELEVATE";
  const accentText = cms?.accentText || "EVERYDAY.";
  const welcomeText = cms?.welcomeText || "Welcome to";
  const description = cms?.description || "A convergence of bakery, gaming, and elite lifestyle services.";
  const ctaPrimary = cms?.ctaPrimary || "Explore Ecosystem";
  const ctaSecondary = cms?.ctaSecondary || "Our Vision";
  const customHeroImage = cms?.heroImage;
 ...
 const scale = useTransform(scrollYProgress, [0, 0.8], [1, 1.15]);
  ...
  <motion.img 
   ...
   onError={(e) => {
     ...
     target.src = lightHeroImg;
     target.classList.add("brightness-50", "contrast-125");
   }}
 ...
 >
 <span className="text-foreground font-black tracking-[0.4em] uppercase text-sm block drop-shadow-[0_0_12px_rgba(0,0,0,0.5)]">
 {welcomeText} <span className="text-primary uppercase">Orient</span>
 </span>
 </motion.div>

 <div className="font-heading font-black text-5xl sm:text-6xl md:text-8xl lg:text-[8.5rem] text-foreground tracking-tighter leading-[0.8] uppercase flex flex-col items-center gap-2">
 <motion.div
 ...
 className="inline-block"
 >
 <span className="bg-gradient-to-r from-primary from-[0%] via-primary via-[60%] to-transparent bg-clip-text text-transparent">
 {accentText}
 </span>
 </motion.div>
 </div>
 ...
 className="relative z-10 px-6 py-4 rounded-2xl bg-card/10 backdrop-blur-md dark:backdrop-blur-none border border-transparent shadow-lg dark:shadow-none"
 >
 <p className="text-base sm:text-lg md:text-xl lg:text-2xl text-foreground max-w-4xl mx-auto font-light tracking-wide leading-relaxed drop-shadow-xl dark:drop-shadow-lg text-center">
 {description}
 </p>
 </motion.div>
 </div>
...
const App: React.FC = () => {
 ...
 let freeScrollTimeout: NodeJS.Timeout;
 ...
 const snapToIndex = (index: number, duration = 1.8) => {
  ...
  const handleWheel = (e: WheelEvent) => {
    ...
    // Allow natural scroll in footer
    if (rawIndex >= 11.95) {
      if (e.deltaY > 0) {
        return; // Scrolling down in footer, allow natural
      } else if (e.deltaY < 0 && rawIndex > 12.05) {
        return; // Scrolling up in footer but not at the top, allow natural
      }
    }
    ...
    // Fast-scroll engages ONLY on continuous multi-event spinning (continuous rapid burst of 5+ events AND cumulative delta > 320)
    const isSustainedFastScroll = consecutiveRapidEvents >= 5 && Math.abs(wheelAccumulator) > 320;

    if (isSustainedFastScroll || isFreeScrolling) {
      stopActiveAnimation();
      isFreeScrolling = true;
      wheelAccumulator = 0;

      clearTimeout(freeScrollTimeout);
      freeScrollTimeout = setTimeout(() => {
        isFreeScrolling = false;
        consecutiveRapidEvents = 0;
      }, 180);

      e.preventDefault();
      // Controlled fast scrolling with 20% reduction (0.80 multiplier)
      container.scrollTop += e.deltaY * 0.80;
      return;
    }

    // If an animation is already in flight, prevent conflicting micro-drifts
    if (isSnapping) {
      e.preventDefault();
      return;
    }

    // Small or normal intentional movement -> smoothly glide to next section
    // Low threshold (6px) so even a gentle flick registers immediately and advances forward!
  ...
  const handleScroll = () => {
    ...
    // When fast/free scrolling finishes, settle into section (NEVER catapult backwards!)
    clearTimeout(scrollTimeout);
    scrollTimeout = setTimeout(() => {
      if (currentView === 'home' && !isFreeScrolling && !isSnapping) {
        ...
        if (!isPerfectlyAligned && !isInsideFooter) {
          const frac = currentRaw - Math.floor(currentRaw);
          let target = Math.round(currentRaw);

          // Direction-aware alignment: always complete progress in the direction scrolled!
          // NEVER bounce or catapult backwards against scroll direction!
          if (direction === 'down') {
  ...
  const handleTouchMove = (e: TouchEvent) => {
    ...
    if (rawIndex >= 11.95) return; // Allow natural swipe near footer
    ...
    // Fast scroll on touch: only on massive continuous drag (> 350px)
    const isSustainedFastTouch = Math.abs(touchAccumulator) > 350;

    if (isSustainedFastTouch || isFreeScrolling) {
      stopActiveAnimation();
      isFreeScrolling = true;
      touchAccumulator = 0;
      ...
      e.preventDefault();
      container.scrollTop += deltaY * 0.80;
      return;
    }
    ...
    // Gentle or small swipe -> smoothly glide to next section
 ...
 {currentView === 'home' && (
 <>
 <Navbar setCurrentView={setCurrentView} scrolled={scrolled} navHidden={navHidden} isReady={!isLoading} skipAnimation={hasPlayedInitialAnimation.current} setCurrentSectionIndex={setCurrentSectionIndex} currentSectionIndex={currentSectionIndex} pageType="hero" heroId="hero" />
 ...
 <StudioApp onCancel={() => setCurrentView('home')} initialRoute={currentView === 'admin' ? '/admin-login' : '/login'} />
 )}
 {currentView === 'bakery' && (
