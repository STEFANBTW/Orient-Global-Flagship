...
const DashboardScreen: React.FC = () => {
 ...
 {/* Sidebar */}
 <aside className="hidden md:flex flex-col w-60 h-[calc(100vh-4rem)] sticky top-0 lg:top-12 bg-foreground text-background border-r border-primary/10 z-50">
 <div className="p-6 flex items-center gap-3">
 <span className="material-icons text-primary text-2xl">restaurant</span>
 <h1 className="text-lg font-bold tracking-wider text-background">ORIENT</h1>
 </div>
 ...
 <div className="p-5 border-t border-transparent">
 ...
 <div>
 <p className="text-xs font-medium text-background">Elias Doe</p>
 ...
 {/* Mobile Header */}
 <header className="md:hidden flex items-center justify-between p-4 bg-foreground text-background border-b border-transparent sticky top-0 lg:top-12 z-50">
 <div className="flex items-center gap-2">
 <span className="material-icons text-primary">restaurant</span>
 <span className="font-bold text-background">ORIENT</span>
 </div>
 <button className="text-background"><span className="material-icons">menu</span></button>
 </header>
 ...
 <div>
 <h2 className="text-2xl font-bold text-background mb-1">Loyalty Dashboard</h2>
 <p className="text-muted-foreground text-sm">Welcome back, Elias. You're just 250 points away from Platinum.</p>
 </div>
 <div className="flex gap-2">
 <button className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-foreground text-background border border-transparent hover:border-primary/50 text-xs text-background/80 transition-all">
 <span className="material-icons text-xs">qr_code</span> Show Card ID
 </button>
 <button className="relative p-1.5 rounded-lg bg-foreground text-background border border-transparent hover:text-primary transition-colors">
 ...
 <div className="relative bg-gradient-to-135 from-[#1a110c] to-[#38251b] rounded-2xl p-6 h-full min-h-[260px] flex flex-col justify-between shadow-2xl shadow-black/50 border border-transparent overflow-hidden">
 ...
 <div>
 <h3 className="text-background/80 uppercase tracking-[0.2em] text-[10px] mb-1">Orient Rewards</h3>
 <div className="flex items-center gap-2">
 ...
 <span className="text-5xl font-bold text-background tracking-tighter">1,250</span>
 ...
 <div className="flex justify-between items-end mt-3 text-[10px] text-background/30 font-mono z-10">
 ...
 <h3 className="text-lg font-bold text-background border-l-4 border-primary pl-3">Points to Perks</h3>
 <a href="#" className="text-xs text-primary hover:text-background transition-colors">View All Rewards -&gt;</a>
 </div>
 ...
 />
 <div className="bg-foreground text-background rounded-xl overflow-hidden group border border-transparent opacity-70 hover:opacity-100 transition-opacity">
 ...
 <span className="material-icons text-3xl text-background/50">lock</span>
 </div>
 <span className="absolute top-2.5 right-2.5 bg-card/10 backdrop-blur-md text-background/70 text-[10px] font-bold px-2.5 py-0.5 rounded-full border border-transparent">2000 PTS</span>
 </div>
 <div className="p-5">
 <h4 className="text-base font-bold text-background/80 mb-1.5">VIP Table Booking</h4>
 <p className="text-muted-foreground text-xs mb-3">Priority reservation for the best seat in the house with champagne.</p>
 <div className="relative w-full h-1.5 bg-card/5 rounded-full mb-3">
 <div className="absolute top-0 left-0 h-full bg-primary/50 w-[62%] rounded-full"></div>
 </div>
 <button className="w-full py-2 rounded-lg bg-card/5 text-muted-foreground text-xs font-medium cursor-not-allowed flex items-center justify-center gap-2" disabled>
 750 Points Needed
 </button>
 </div>
 </div>
 ...
 <section>
 <div className="flex justify-between items-center mb-5">
 <h3 className="text-lg font-bold text-background border-l-4 border-primary pl-3">Recent Dining History</h3>
 <button className="p-1 text-muted-foreground hover:text-background"><span className="material-icons text-sm">filter_list</span></button>
 </div>
 <div className="bg-foreground text-background rounded-xl overflow-hidden border border-transparent">
 <div className="overflow-x-auto">
 <table className="w-full text-left border-collapse">
 <thead>
 <tr className="bg-card/5 text-muted-foreground text-[10px] uppercase tracking-wider">
 ...
 <tbody className="divide-y divide-white/5 text-xs text-background/80">
 ...
 <div className="p-3 border-t border-transparent text-center">
 <button className="text-xs text-muted-foreground hover:text-primary transition-colors">Load More History</button>
 </div>
 </div>
 </section>
...
const NavItem = ({ icon, label, active = false }: { icon: string, label: string, active?: boolean }) => (
 <a href="#" className={`flex items-center gap-3 px-3 py-2 rounded-lg transition-colors ${active ? 'text-background bg-primary/20 border-l-4 border-primary' : 'text-muted-foreground hover:text-background hover:bg-card/5'}`}>
...
const StatCard = ({ icon, value, label, color }: { icon: string, value: string, label: string, color: 'primary' | 'green' }) => (
 <div className="glass-card p-4 rounded-xl flex flex-col justify-center items-center text-center hover:bg-card/5 transition-colors cursor-pointer group">
 ...
 <h4 className="text-base font-bold text-background">{value}</h4>
 <p className="text-[10px] text-muted-foreground">{label}</p>
 </div>
);

const RewardCard = ({ image, points, title, desc }: { image: string, points: number, title: string, desc: string }) => (
 <div className="bg-foreground text-background rounded-xl overflow-hidden group border border-transparent hover:border-primary/30 transition-all duration-300">
 <div className="h-40 overflow-hidden relative">
 <img src={image} alt={title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />
 <div className="absolute inset-0 bg-gradient-to-t from-surface-dark to-transparent"></div>
 <span className="absolute top-2.5 right-2.5 bg-black/60 backdrop-blur-md text-background text-[10px] font-bold px-2.5 py-0.5 rounded-full border border-transparent">{points} PTS</span>
 </div>
 <div className="p-5">
 <h4 className="text-base font-bold text-background mb-1.5">{title}</h4>
...
const HistoryRow = ({ date, location, items, spent, points }: { date: string, location: string, items: string, spent: string, points: string }) => (
 <tr className="hover:bg-card/5 transition-colors group">
 <td className="p-3">{date}</td>
 <td className="p-3 flex items-center gap-2">
 <span className="material-icons text-[10px] text-primary">place</span> {location}
 </td>
 <td className="p-3 text-muted-foreground">{items}</td>
 <td className="p-3 font-medium text-background">{spent}</td>
 <td className="p-3 text-right text-primary font-bold">{points}</td>
 <td className="p-3 text-center">
 <button className="text-[10px] font-medium text-primary hover:text-background border border-primary/30 hover:bg-primary px-2.5 py-1 rounded transition-all">Reorder</button>
 </td>
 </tr>
);
