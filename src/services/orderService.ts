...
import { getActiveConsumerUser } from './mockUsers';
...
export type OrderStatus = 'awaiting_chef' | 'confirmed' | 'preparing' | 'ten_min_warning' | 'ready' | 'completed' | 'cancelled';
...
export interface AppNotification {
  id: string;
  orderId?: string;
  type: 'order_placed' | 'order_confirmed' | 'ten_min_warning' | 'order_ready' | 'order_cancelled' | 'info';
...
export const orderService = {
  ...
  placeOrder: async (input: {
    ...
    const newOrder: CustomerOrder = {
      id: orderId,
      customerId: input.customerId?.trim() || activeUser.id,
      customerName: input.customerName?.trim() || activeUser.name,
      customerEmail: input.customerEmail?.trim() || activeUser.email,
      customerPhone: input.customerPhone?.trim() || activeUser.phone,
      tableNumber: input.tableNumber?.trim() || 'Takeout / Pickup',
      shippingAddress: input.shippingAddress?.trim() || activeUser.deliveryAddress,
      notes: input.notes?.trim() || '',
      items: sanitizedItems,
      totalAmount,
      status: 'awaiting_chef',
      prepDurationMinutes: input.prepDurationMinutes || 25,
      timerEndsAt: null,
  ...
  confirmAndStartOrder: async (orderId: string, prepMinutes?: number): Promise<CustomerOrder> => {
    ...
    const prepDuration = prepMinutes || order.prepDurationMinutes || 25;
