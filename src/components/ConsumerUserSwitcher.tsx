...
export function ConsumerUserSwitcher() {
  const [currentUser, setCurrentUser] = useState<MockConsumerUser>(getActiveConsumerUser());
  ...
  const loadCustomerOrders = async () => {
    try {
      ...
      // Match by customerId or customerName/phone
  ...
  return (
    <>
      <div className="flex items-center gap-2">
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button 
              ...
              className="flex items-center gap-2 px-3 py-1.5 rounded-xl border border-border bg-muted/60 hover:bg-muted text-foreground transition-colors text-xs font-medium"
            >
          ...
          <DropdownMenuContent align="end" className="w-72 bg-card border-border">
            <DropdownMenuLabel className="text-xs font-semibold text-foreground">
              Switch Consumer Account (Nigeria +234)
            </DropdownMenuLabel>
            <p className="px-2 pb-2 text-[11px] text-muted-foreground">
              Select a user to test live customer views without entering a password.
            </p>
            <DropdownMenuSeparator />
        ...
        {/* Customer Live Tracker Button */}
        <Button
          id="btn-open-customer-tracker"
          size="sm"
          variant="outline"
          onClick={() => setIsTrackerOpen(true)}
          className="relative h-8 px-2.5 text-xs font-medium border-border text-foreground hover:bg-muted gap-1.5"
        >
        ...
        <DialogContent className="sm:max-w-lg border-border">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-base font-bold text-foreground">
              ...
              Customer Orders: {currentUser.name}
            </DialogTitle>
