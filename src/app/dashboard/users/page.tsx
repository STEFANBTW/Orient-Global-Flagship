...
export default function UsersPage() {
 ...
 <Button variant="outline" className="bg-card border-transparent text-xs h-9">
 Export
 </Button>
 ...
 className="bg-card/50 border border-transparent rounded-xl overflow-hidden shadow-2xl"
 >
 <div className="p-4 border-b border-transparent bg-background/30">
 <div className="relative max-w-md">
 <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
 <Input 
 placeholder="Search users..." 
 className="pl-10 bg-background border-transparent h-10 text-sm"
 />
 </div>
 </div>
 <Table>
 ...
 <TableBody>
 {users.map((user) => (
 <TableRow key={user.id} className="border-transparent hover:bg-card/[0.01] transition-colors group">
 <TableCell className="py-4">
 <div className="flex items-center gap-3">
 <Avatar className="h-9 w-9 border border-transparent bg-primary/20 text-primary font-bold text-xs shrink-0">
 <AvatarFallback>{user.avatar}</AvatarFallback>
 </Avatar>
 <div className="flex flex-col min-w-0">
 <span className="text-sm font-semibold text-foreground group-hover:text-primary transition-colors truncate max-w-[150px]">{user.name}</span>
 ...
 <span className="text-xs text-muted-foreground/50 font-medium">{user.status}</span>
 </div>
 </TableCell>
