...
const BakeryMenuItem: React.FC<{ title: string; price: string; desc: string; image: string; ingredients: string }> = ({ title, price, desc, image, ingredients }) => (
  <div className="group relative bg-white dark:bg-stone-900 rounded-xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-500 break-inside-avoid mb-6">
    <div className="relative aspect-[4/3] md:aspect-auto overflow-hidden">
      ...
      <div className="absolute top-3 right-3 bg-white/90 dark:bg-background-dark/90 backdrop-blur text-xs font-bold px-3 py-1 rounded-full text-primary shadow-sm">{price}</div>
      <div className="absolute inset-0 bg-background-dark/80 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 p-6 text-center">
        <div>
    ...
    <div className="p-6">
      <div className="flex justify-between items-start mb-2">
        <h3 className="font-serif text-2xl text-slate-900 dark:text-white font-bold">{title}</h3>
...
const BakeryPairingCard: React.FC<{ title: string; desc: string; image: string }> = ({ title, desc, image }) => (
  <motion.div whileHover={{ y: -10 }} className="bg-white dark:bg-stone-900 rounded-3xl overflow-hidden shadow-lg border border-stone-100 dark:border-stone-700 group transition-all">
...
export const BakeryHome: React.FC = () => {
  ...
  return (
    <div className="bg-slate-50 dark:bg-background-dark overflow-x-hidden pt-0">
      ...
      {/* Daily Menu Section */}
      <section className="py-20 bg-slate-50 dark:bg-background-dark relative" id="menu">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
            <div>
              <h2 className="font-serif text-4xl text-slate-900 dark:text-white font-bold mb-2">Daily Selection</h2>
            ...
            <div className="bg-white dark:bg-stone-900 p-1.5 rounded-full shadow-sm border border-stone-200 dark:border-stone-700 flex">
              {categories.map((cat) => (
                <button key={cat} onClick={() => setActiveCategory(cat)} className={`px-6 py-2 rounded-full text-sm font-medium transition-all ${ activeCategory === cat ? 'bg-background-dark text-white shadow-md' : 'text-stone-600 dark:text-stone-300 hover:bg-stone-100 dark:hover:bg-stone-700' }`}>
                  {cat}
                </button>
              ))}
            </div>
...
export const BakeryNav: React.FC<{ navHidden: boolean, currentView: string, setView: (v: 'home' | 'architect' | 'wholesale' | 'story') => void, localTheme?: 'dark'|'light', toggleLocalTheme?: () => void }> = ({ navHidden, currentView, setView, localTheme, toggleLocalTheme }) => {
  ...
  return (
    <motion.div
      ...
      className="fixed w-full z-40 top-0 bg-white/80 dark:bg-background-dark/80 backdrop-blur-xl border-b border-stone-200 dark:border-white/10"
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
