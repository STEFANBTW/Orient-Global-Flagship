...
export const SupermarketNav: React.FC<{ navHidden: boolean, activePage: SupermarketPage, setActivePage: (p: SupermarketPage) => void, setIsSmartPasteOpen: (o: boolean) => void, localTheme?: 'dark'|'light', toggleLocalTheme?: () => void }> = ({ navHidden, activePage, setActivePage, setIsSmartPasteOpen, localTheme, toggleLocalTheme }) => {
  ...
  return (
    <motion.nav 
      ...
      className="bg-slate-900/90 backdrop-blur-xl text-white border-b border-white/10 fixed top-0 w-full z-40 shadow-md"
    >
          ...
          <div className="flex items-center gap-1 flex-1 overflow-x-auto no-scrollbar px-2">
            {pages.map((page) => (
              <button
                ...
                className={`relative flex items-center gap-2 px-3 py-1.5 rounded-full text-[10px] sm:text-xs font-medium transition-all whitespace-nowrap z-10 ${
                  activePage === page.id
                    ? 'text-slate-900'
            ...
            <motion.button
              ...
              className="flex items-center gap-2 px-3 py-1.5 rounded-full text-[10px] sm:text-xs font-medium transition-all whitespace-nowrap text-slate-400 hover:text-white hover:bg-slate-800"
            >
...
export const SupermarketApp: React.FC<{ activePage: SupermarketPage, setActivePage: (p: SupermarketPage) => void, isSmartPasteOpen: boolean, setIsSmartPasteOpen: (o: boolean) => void }> = ({ activePage, setActivePage, isSmartPasteOpen, setIsSmartPasteOpen }) => {
  const renderContent = () => {
    ...
    return (
      <div className="min-h-screen flex flex-col font-sans relative bg-slate-50 dark:bg-slate-950 transition-colors duration-300 pt-36">
